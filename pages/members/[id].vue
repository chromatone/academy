...
const { data: member, error } = await useAsyncData('member-' + route.params?.id, async () => await getItemById({
  ...
  id: route.params?.id,
  params: {
    fields: ['active', 'role', 'user', 'team.*', 'team.programs.programs_id.title', 'team.programs.programs_id.slug', 'team.programs.position', 'team.courses.courses_slug.title', 'team.courses.courses_slug.slug', 'team.courses.position',
    ]
  }
}))
